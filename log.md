...
- 首页设计升级：实现两个“大卡片”首页，分别介绍学习模式与背诵模式，卡片采用左侧为标题+放大图标，右侧为描述和要点的布局，偶数卡片实现左右交错排布，单卡占据整行，提升视觉层级与可读性。
- UI 与动效：增加卡片进入动画、描述文本逐行呈现动画，图标尺寸进一步放大以提升辨识度，边距与填充也整体增大，提升可读性与美观性。
- CTA 改造：开始学习/开始背诵按钮改为使用 shadcn 的 Button 组件，点击进入对应学习入口。
- 首页引导：新增 HomeGuide 组件，放置在首页卡片前，提供简短的引导文本，帮助新用户快速理解两种模式的定位。
- 代码结构优化：新增 home-mode-card.tsx、home-guide.tsx，重构首页以更易扩展的方式管理卡片内容与引导文本。
- 可访问性：为卡片添加 aria-label 提升无障碍。

- 变更文件：
-  src/components/home/home-mode-card.tsx
-  src/components/home/home-guide.tsx
-  src/app/page.tsx

## Why
提升首次打开的引导性和可用性，使新玩家能够快速理解两种学习路径的定位与入口。

## How to Test
- 打开首页 /，看到两张“大卡片”，且单卡占据整行
- 验证移动端单列、桌面端两列或单列的自适应表现，以及文本逐行呈现的动画效果。
- 点击进入学习模式/背诵模式的入口，进入 /learn?mode=learn 和 /learn?mode=recite。

## Next Steps
- 如需对文案/图标进行进一步迭代，或增加 A/B 测试，请告知。可继续完善单元测试与回归测试用例。
